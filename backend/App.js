...
app.use(express.static(path.join('public')));   //this line added to serve our react app files statically, handles any static files such as JS files
                                                //directs to our 'public' folder where frontend code is

// app.use((req, res, next) => {                                    //CORS headers are no longer needed when serving both frontend and backend from the same host
...
app.use('/api/places', placesRoutes);   //these two routes handle backend requests
app.use('/api/users', usersRoutes);     //any request not handled by these routes, will be routed to the 'public' folder below

app.use((req, res, next) => {
    res.sendFile(path.resolve(__dirname, 'public', 'index.html'));  //if requests are not static files or backend routes, this statement directs to our single page
});   

// app.use((req, res, next) => {
//     const error = new HttpError('Could not find this route.', 404);  //this middleware handles unknown requests but is no longer needed because the previous line was added 
//     throw error;
// });
